...
describe('ConfigurationDetailsEffects', () => {
  ...
  describe('loadConfigurationById$', () => {
    ...
    it('should dispatch configurationDetailsReceived when id is undefined', (done) => {
      const resource = { id: '1', name: 'c' } as Configuration
      configurationService.getConfiguration.mockReturnValue(of({ ...resource }) as any)
  ...
  describe('saveButtonClicked$', () => {
    ...
    it('should cancel update when details is null', (done) => {
      store.overrideSelector(configurationDetailsSelectors.selectDetails, null as any)
      store.refreshState()
      actions$.next(ConfigurationDetailsActions.saveButtonClicked({ details: {} as any }))
      effects.saveButtonClicked$.subscribe((action) => {
        expect(action).toEqual(ConfigurationDetailsActions.updateConfigurationCancelled())
  ...
  describe('deleteButtonClicked$', () => {
    ...
    it('should dispatch deleteConfigurationFailed when item has no id', (done) => {
      portalDialogService.openDialog.mockReturnValue(of({ button: 'primary' } as DialogState<any>) as any)
      store.overrideSelector(configurationDetailsSelectors.selectDetails, { id: undefined } as any)
      store.refreshState()
      actions$.next(ConfigurationDetailsActions.deleteButtonClicked())
    ...
    it('should throw error when itemToDelete is null', (done) => {
      portalDialogService.openDialog.mockReturnValue(of({ button: 'primary' } as DialogState<any>) as any)
      store.overrideSelector(configurationDetailsSelectors.selectDetails, null as any)
      store.refreshState()
      actions$.next(ConfigurationDetailsActions.deleteButtonClicked())
  ...
  describe('loadMCPServers$', () => {
    it('should dispatch configurationMCPServersReceived on success', (done) => {
      const mockMCPServers = [{ id: 'mcp-1', name: 'MCP Server 1' }]
      mcpServerService.findMCPServerByCriteria.mockReturnValue(of({ stream: mockMCPServers }) as any)
      actions$.next(ConfigurationDetailsActions.navigatedToDetailsPage({ id: '1' }))
      effects.loadMCPServers$.subscribe((action) => {
        expect(action).toEqual(
          ConfigurationDetailsActions.configurationMCPServersReceived({ MCPServers: mockMCPServers })
        )
        done()
      })
    })
    it('should dispatch configurationMCPServersLoadingFailed on error', (done) => {
      const err = 'mcp error'
      mcpServerService.findMCPServerByCriteria.mockReturnValue(throwError(() => err) as any)
      actions$.next(ConfigurationDetailsActions.navigatedToDetailsPage({ id: '1' }))
      effects.loadMCPServers$.subscribe((action) => {
        expect(action).toEqual(ConfigurationDetailsActions.configurationMCPServersLoadingFailed({ error: err }))
        done()
      })
    })
  })
  ...
  describe('loadProviders$', () => {
    it('should dispatch configurationProvidersReceived on success', (done) => {
      const mockProviders = [{ id: 'p-1', name: 'Provider 1' }] as any[]
      providerService.findProviderBySearchCriteria.mockReturnValue(of({ stream: mockProviders }) as any)
      actions$.next(ConfigurationDetailsActions.navigatedToDetailsPage({ id: '1' }))
      effects.loadProviders$.subscribe((action) => {
        expect(action).toEqual(ConfigurationDetailsActions.configurationProvidersReceived({ providers: mockProviders }))
        done()
      })
    })
    it('should dispatch configurationProvidersLoadingFailed on error', (done) => {
      const err = 'provider error'
      providerService.findProviderBySearchCriteria.mockReturnValue(throwError(() => err) as any)
      actions$.next(ConfigurationDetailsActions.navigatedToDetailsPage({ id: '1' }))
      effects.loadProviders$.subscribe((action) => {
        expect(action).toEqual(ConfigurationDetailsActions.configurationProvidersLoadingFailed({ error: err }))
        done()
      })
    })
  })
  ...
  describe('cancelButtonNotDirty$', () => {
    it('should dispatch cancelEditNotDirty when cancelButtonClicked with dirty: false', (done) => {
    ...
    it('should display error message on configurationDetailsLoadingFailed', (done) => {
      ...
      effects.displayError$.subscribe(() => {
        expect(errorSpy).toHaveBeenCalledWith({
          summaryKey: 'CONFIGURATION_DETAILS.ERROR_MESSAGES.DETAILS_LOADING_FAILED'
        })
        done()
      })
    })
